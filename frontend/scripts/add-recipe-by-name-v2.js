 ...
 * 
 * Version 2 : Amélioration de la détection des ingrédients existants
 */
...
async function main() {
  try {
    ...
    // Demander à l'utilisateur quelle recette traiter si plusieurs sont disponibles
    let recipeIndex = 0;
    if (recipes.length > 1) {
      ...
      const answer = await askQuestion(rl, 'Quelle recette souhaitez-vous traiter? (numéro): ');
      recipeIndex = parseInt(answer) - 1;
      
      if (isNaN(recipeIndex) || recipeIndex < 0 || recipeIndex >= recipes.length) {
        console.error('Numéro de recette invalide.');
        rl.close();
        return;
      }
    }
    
    const recipeData = recipes[recipeIndex];
    console.log(`\nTraitement de la recette: ${recipeData.name}`);
    ...
    // Si aucun ingrédient ou ustensile valide n'est trouvé, ajouter tous les ingrédients et ustensiles de la recette
    if (validIngredients.length === 0 || validKitchenTools.length === 0) {
      console.log('\nAucun ingrédient ou ustensile valide trouvé dans la base de données.');
      console.log('Tous les ingrédients et ustensiles de la recette seront ajoutés.');
      
      const missingIngredients = recipeData.ingredients;
      const missingKitchenTools = recipeData.kitchen_tools.map(tool => tool.name);
      ...
      if (missingIngredients.length > 0) {
        sql += '-- Ajout des ingrédients\n';
        
        for (const ingredient of missingIngredients) {
          console.log(`\nAjout de l'ingrédient: ${ingredient.name}`);
      ...
      if (missingKitchenTools.length > 0) {
        sql += '-- Ajout des ustensiles\n';
        
        for (const toolName of missingKitchenTools) {
          console.log(`\nAjout de l'ustensile: ${toolName}`);
      ...
      // Exécuter le SQL pour ajouter les ingrédients et ustensiles
      if (sql !== '') {
      ...
      console.log(`Nombre d'ingrédients valides après ajout: ${validUpdatedIngredients.length}`);
      console.log(`Nombre d'ustensiles valides après ajout: ${validUpdatedKitchenTools.length}`);
      
      // Si toujours aucun ingrédient ou ustensile valide n'est trouvé, abandonner
      if (validUpdatedIngredients.length === 0 || validUpdatedKitchenTools.length === 0) {
        console.error('Impossible de récupérer les ingrédients et ustensiles de la base de données.');
        console.error('La recette ne peut pas être ajoutée.');
        rl.close();
        return;
      }
      
      // Continuer avec les ingrédients et ustensiles mis à jour
      ingredients = validUpdatedIngredients;
      kitchenTools = validUpdatedKitchenTools;
    }
    ...
    // Vérifier que tous les ingrédients et ustensiles sont maintenant disponibles
    ...
    if (!allItemsAvailable) {
      console.error('Certains ingrédients ou ustensiles ne sont pas disponibles dans la base de données.');
      console.error('La recette ne peut pas être ajoutée.');
      rl.close();
      return;
    }
